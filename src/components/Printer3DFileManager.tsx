...
export const Printer3DFileManager = ({ 
  ...
  const generateGCodeFromConfig = (models: any[]): string => {
    ...
    models.forEach((model, index) => {
      gcode += `; Model ${index + 1}: ${model.filename}\n`;
      gcode += `; Position: X${model.position[0]} Y${model.position[1]} Z${model.position[2]}\n`;
      gcode += `; Rotation: X${model.rotation[0]} Y${model.rotation[1]} Z${model.rotation[2]}\n`;
      gcode += `; Scale: X${model.scale[0]} Y${model.scale[1]} Z${model.scale[2]}\n\n`;
    });
